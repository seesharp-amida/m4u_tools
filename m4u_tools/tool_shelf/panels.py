...
def layout_armature(col, prop_target, amature_prop_name, bone_prop_name = None):
  col.prop_search(prop_target, amature_prop_name, bpy.data, "armatures", text="Armature")
  arma = bpy.data.armatures.get(getattr(prop_target, amature_prop_name))
  if bone_prop_name:
    if arma is not None:
      col.prop_search(prop_target, bone_prop_name, arma, "bones", text="Bone")
...
class M4_Panel_Bone(MechTab, bpy.types.Panel):
  ...
  def draw(self, context):
    ...
    col = layout.column()
    layout_armature(col, mech_model, "set_bone_armature_name", "set_bone_bone_name")
